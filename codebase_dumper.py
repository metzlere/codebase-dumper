...
Usage:
    python codebase_dumper.py --root-dir ./src --output-file codebase_dump.md

For additional options and usage details, run:
    python codebase_dumper.py --help
"""
...
def generate_directory_tree(root_dir: str, ignored_dirs: List[str]) -> str:
    """
    Generates a visual directory tree structure for the codebase.
...
def scan_codebase(
    ...
    """
    Scans the codebase and extracts the contents of files based on extension and filtering criteria.
...
def main() -> None:
    """
    Parses command-line arguments, generates the directory tree and code dump,
    and writes the output to a Markdown file.
    """
    parser = argparse.ArgumentParser(
        description="Dump a codebase's directory structure and contents into a Markdown file for LLM prompts."
    )
    parser.add_argument(
        "--root-dir", type=str, default="./src",
        help="Root directory of the codebase to scan (default: './src')."
    )
    parser.add_argument(
        "--output-file", type=str, default="codebase_dump.md",
        help="Path to the output Markdown file (default: 'codebase_dump.md')."
    )
    parser.add_argument(
    ...
    print(f"Codebase with directory structure dumped to {args.output_file}")
